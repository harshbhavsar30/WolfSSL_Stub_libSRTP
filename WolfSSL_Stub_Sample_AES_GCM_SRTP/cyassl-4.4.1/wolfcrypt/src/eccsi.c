 ...
 * Flags that the order is avaialble so it isn't loaded multiple times.
 *
 ...
 * Flags that each parameter is avaialble so they aren't loaded multiple times.
 *
 * @param  [in]  key  ECCSI key.
 * @return  0 on success.
 * @return  MEMORY_E when dynamic memory allocation fails.
 ...
 *
 * Flags that base is avaialble so it isn't loaded multiple times.
 ...
 * @return  MEMORY_E when dynamic memory allocation fails (WOLFSSL_SMALL_STACK).
 */
int wc_EncodeEccsiSsk(EccsiKey* key, mp_int* ssk, byte* data, word32* sz)
{
    int err = 0;
