...
#endif
        {
            err = wc_ecc_make_key_ex(rng, key->dp->size, pubkey, key->dp->id);
        }
        if (err != MP_OKAY) break;
...
#endif
{
  ...
  if (err == MP_OKAY)
    ...
    err = ecc_projective_dbl_point(precomp[4], precomp[8], a, modulus, mp);

  if (err == MP_OKAY)
    err = ecc_projective_add_point(precomp[4], precomp[8], precomp[12],
                                   a, modulus, mp);
