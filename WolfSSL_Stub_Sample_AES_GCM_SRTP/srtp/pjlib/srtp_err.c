...
srtp_err_reporting_level_t err_level = srtp_err_level_debug; // srtp_err_level_error;
...
void srtp_err_report(srtp_err_reporting_level_t priority, const char *format, ...)
{
	...
	if (priority <= err_level) {
		...
		//KGK-SRTP-SUPPORT :- Print the logs appropritely
		//pj_log("libsrtp", 3/*priority*/, format, args);
		//printf(format, args);
		va_end(args);
	}
#endif
