...
/**
 * Get Current Ithread id.
 *
 * @param			None
 *
 * @return	        Ithread id.
 */
void *pj_get_current_thread_id();
#endif
/**
 * Create a new thread.
 *
...
PJ_DECL(pj_status_t) pj_thread_create(  pj_pool_t *pool, 
					...
					pj_thread_t **thread );
#ifdef POC_CDE_BREW
PJ_DECL(void *) pj_thread_internal(pj_thread_t *thread);
#endif
 ...
 * @return	    PJ_SUCCESS on success, or the error code.
 */
#if (defined(PJ_ANDROID) && PJ_ANDROID!=0) || (defined KN_IOS_BTR_OPT)
#define pj_mutex_lock(mutex) pj_mutex_lock_debug(mutex, __FILE__, __LINE__)
PJ_DECL(pj_status_t) pj_mutex_lock_debug(pj_mutex_t *mutex, char *fileName, int lineNum);
#else
