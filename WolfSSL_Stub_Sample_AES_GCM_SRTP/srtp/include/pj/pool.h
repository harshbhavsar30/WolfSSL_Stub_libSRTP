...
struct pj_caching_pool 
{
	...
	/* START - [64_BIT_PORTING :  : Crash due to size] */
#ifdef POC_CDE_BREW
    char	    pool_buf[256 * (sizeof(pj_size_t) / 4)];
#elif (defined(PJ_ANDROID) && PJ_ANDROID!=0) || (defined KN_IOS_BTR_OPT)
	char	    pool_buf[512 * (sizeof(size_t) / 4)];
#else
    char	    pool_buf[256 * (sizeof(size_t) / 4)];
#endif
	/* END - [64_BIT_PORTING :  : Crash due to size] */
