...
#if defined(PJ_MTK) && PJ_MTK != 0
//void psptt_assert(int expr);
void psptt_assert(char* expr, char* file, int line);
#endif
...
#ifndef pj_assert
#if defined(PJ_MTK) && PJ_MTK != 0
# define pj_assert(expr) do {if(!(expr)) psptt_assert((char*)#expr, (char*)__FILE__, __LINE__); } while (0)
#elif defined POC_CDE_BREW || (defined(PJ_THREADX) && PJ_THREADX!= 0)
# define pj_assert(expr)  psptt_assert(expr)
#else
