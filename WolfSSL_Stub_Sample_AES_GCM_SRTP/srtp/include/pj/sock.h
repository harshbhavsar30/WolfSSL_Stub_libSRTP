...
/*NwkTsn Changes for Eastwood - Begin*/
#if defined(PJ_THREADX)
extern const pj_uint16_t PJ_SOCK_IPC_DGRAM;
#endif
/*NwkTsn Changes for Eastwood - End*/
...
/*NwkTsn Changes for Eastwood - Begin*/
#if defined(PJ_THREADX)
# define pj_SOCK_IPC_DGRAM()	PJ_SOCK_IPC_DGRAM 
# define pj_SOCK_IPC_STREAM() 	PJ_SOCK_IPC_STREAM
#endif
/*NwkTsn Changes for Eastwood - End*/
...
/** Buffer size for receive. @see pj_SO_TCPWNDSCALE() */
extern const pj_uint16_t PJ_SO_TCPWNDSCALE;
...
/** When this option is set, drop a connection after retransmitting the FIN 3 times. 
	It will prevent holding too many mbufs in socket buffer queues. @see pj_TCP_FINDROP */
extern const pj_uint16_t PJ_TCP_FINDROP;

/** This option is set on an TCP socket to keep the connection alive by refreshing NAT.*/
extern const pj_uint16_t PJ_SO_KEEPALIVE;

/** When SO_KEEPALIVE Option is On, we can configure idle time after which NAT refresh will be performed.*/
...
#   define pj_SO_SNDBUF()   PJ_SO_SNDBUF
   ...
   /** Get #PJ_SO_TCPWND_SCALE constant */
#   define pj_SO_TCPWNDSCALE()  PJ_SO_TCPWNDSCALE

    /** Get #PJ_TCP_NODELAY constant */
#   define pj_TCP_NODELAY() PJ_TCP_NODELAY

	/** Get #PJ_TCP_FINDROP constant */
#   define pj_TCP_FINDROP() PJ_TCP_FINDROP
