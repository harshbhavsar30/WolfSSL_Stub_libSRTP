...
#ifdef POC_CDE_BREW
	void (*ssl_read_cb)( void * key,void *op_key, void *buffer, pj_ssize_t *length, unsigned flags );
	void (*ssl_write_cb)(void *key,void *op_key,const void *data,pj_ssize_t *length,unsigned flags);
#else
	void	*ssl_read_cb;
	void	*ssl_write_cb;
#endif
...
#ifdef KN_IOS_BTR_OPT

PJ_DECL(pj_status_t) pj_activesock_create_sip(pj_pool_t *pool,
                                          pj_sock_t sock,
                                          int sock_type,
                                          const pj_activesock_cfg *opt,
                                          pj_ioqueue_t *ioqueue,
...
long pj_asock_get_fd(pj_activesock_t *asock);

//void KN_asock_Set_Secure_Tp(pj_activesock_t *asock);
...
void Kn_Set_Secure_Tp(pj_ioqueue_t *ioqueue, pj_bool_t flag);

long pj_ioqueue_get_fd(pj_ioqueue_key_t *key);

// KGK-MS-CHANGES  - Start
PJ_DECL(pj_status_t) pj_activesock_set_max_loop(pj_activesock_t *asock, int maxLoop);
// KGK-MS-CHANGES  - End
