...
PJ_IDEF(void*) pj_pool_alloc_from_block( pj_pool_block *block, pj_size_t size )
{
    ...
    if (block && ((pj_size_t)(block->end - block->cur) >= size)) {
	void *ptr = block->cur;
	block->cur += size;
	return ptr;
    }
...
{
	/* KGK - Check for NULL before trying to release the pool */
	if((NULL != pool) && (NULL != pool->factory)  && (NULL != pool->factory->release_pool))
	{
