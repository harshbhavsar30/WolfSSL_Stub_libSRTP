...
{
    str->ptr = (char*)s;
    str->slen = s ? (pj_size_t)pj_ansi_strlen(s) : 0;
    return str;
}
...
{
    str->ptr = src;
    str->slen = src ? (pj_size_t)pj_ansi_strlen(src) : 0;
    return str;
}
...
{   /* START - [64_BIT_PORTING : V106 : <212> ]. i am not touching this boss */
    return (char*) memchr((const void*)str->ptr, chr, str->slen);
	/* END   - [64_BIT_PORTING : V106 : <212> ]. i am not touching this boss */
}
...
* @return the value.
*/
PJ_DEF(double) pj_strtod(const pj_str_t *str);/*E2E_SUPPORT*/
